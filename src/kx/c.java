...
public class c{
  ...
  */
  ...
  Object r() throws UnsupportedEncodingException{
    int i=0, n, t=b[j++];
    if(t<0) //atomic types
      switch(t){ //switch on kdb type
        case -1:
          return rb(); // maps to boolean primative
        case (-2):
          return rg(); // maps to java.util.UUID
        case -4:
          return b[j++]; // maps to message byte
        case -5:
          return rh(); // maps to short primative
        case -6:
          return ri(); // maps to int primative
        case -7:
          return rj();
        case -8:
          return re();
